...
BOOL CGraphDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	//获取控件窗口指针
	CEdit* pEditMin = (CEdit*)GetDlgItem(IDC_EdbAxisMin);
	CEdit* pEditMax = (CEdit*)GetDlgItem(IDC_EdbAxisMax);
	int EDIT_NOW;
	if ((GetFocus() == pEditMin || GetFocus() == pEditMax) && (pMsg->message == WM_CHAR))
	{
		//只允许输入数字和小数点“.”
			...
			|| pMsg->wParam == '.' || pMsg->wParam == '-')
		{
			...
			CString str;
			if (GetFocus() == pEditMin) {
				EDIT_NOW = IDC_EdbAxisMin;
			} else {
				EDIT_NOW = IDC_EdbAxisMax;
			}
			...
			{
				int nPos = 0;
				GetDlgItemText(EDIT_NOW, str); // 获取edit中文本
