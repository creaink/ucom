...
void CUcomDlg::OnBnClickedBtnhelp()
{
	...
	helpInfo = 
"版本：1.1 & 帮助信息：\n\
0.本软件版权所有：长春理工大学电子学会 谢周，技术交流QQ群：560901616\n\
1.自动保存串口、网络配置里的信息和用户设定的背景色和字体色配置\n\
...
16.支持横向、纵向窗口大小调整和发送接收纵向窗口调整（上下拖动发送计数旁按钮）\n\
17.可以提供串口和网络两个数据源，切换数据源，不影响软件的其他功能\n\n\
a.接收图表，支持指定格式数值分拣、图形化显示，格式:空格+通道(0-5)+':'+数据，帧尾换行\n\
     如:“ 0:%d 1:%f\\n”,最多六通，保存的数据文件用Excel打开后另存为表格方便后续分析\n\
     在数据框选择曲线，在右侧的曲线框调整该曲线属性，选择纵横坐标轴分别调整其属性，\n\
     外挂窗口可任意调整大小，在波形图上向右下角拉选取框：放大，反之是还原，右键平移，\n\
     轴范围上大下小，失焦确定；一条曲线默认只缓存最近512个点，建议帧间隔大于20ms\n\
